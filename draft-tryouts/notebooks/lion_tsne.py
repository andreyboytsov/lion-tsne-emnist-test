# THIS IS A COPIED VERSION. AFTER DEBUGGING THE CHANGES WILL BE ADDED TO THE MAIN VERSION.
...
class LionTSNE:
    ...
    def generate_lion_tsne_embedder(self, function_kwargs={}, random_state = None, verbose=0,
        ...
        def result_function(x, verbose=0, additional_output=False):
            x = np.array(x).reshape((-1, self.X.shape[1]))
            ...
            # TODO This is a suboptimal way. But at least we can test it fast.
            # Step 1. Find the distance between outlier samples.
            all_new_points_distances = self.get_distance_matrix(x)
            if verbose >= 2:
                print("Number of outliers: ", len(outlier_samples))
                print("Distance matrix: ", all_new_points_distances)
            outlier_pool = list(outlier_samples)
            outliers_close_to_representatives = dict()
            while len(outlier_pool) > 0:
                if verbose >= 2:
                    print("NEXT OUTLIER. Original pool: ", outlier_pool)
                cur_outlier_representative = outlier_pool.pop()
                if verbose >= 2:
                    print("Testing outlier point ", cur_outlier_representative)
                distance_row = all_new_points_distances[cur_outlier_representative, :]
                distance_selector = distance_row <= radius_x
                distance_selector[cur_outlier_representative] = False # Do not match with itself.
                close_enough_outliers = np.where(distance_selector)[0]
                if verbose >= 2:
                    print("Close outliers: ", close_enough_outliers)
                outliers_close_to_representatives[cur_outlier_representative] = close_enough_outliers
                for i in close_enough_outliers:
                    if i in outlier_pool: # It can get close to a non-outlier point.
                        outlier_pool.remove(i)  # Removing BY VALUE !
                if verbose >= 2:
                    print("Remaining pool: ", outlier_pool)
                    print("Grouped so far: ", outliers_close_to_representatives)
            ...
            if additional_output:
                fraction_outliers = len(outlier_samples) / x.shape[0]
                num_outlier_clusters = len(outliers_close_to_representatives)
                if num_outlier_clusters>0:
                    avg_pts_per_outlier_cluster = len(outlier_samples) / len(outliers_close_to_representatives)
                else:
                    avg_pts_per_outlier_cluster = 0
                return y_result, fraction_outliers, num_outlier_clusters, avg_pts_per_outlier_cluster
            else:
                return y_result
