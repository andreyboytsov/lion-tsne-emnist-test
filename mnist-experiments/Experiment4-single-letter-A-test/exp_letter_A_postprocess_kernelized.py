...
def generate_kernelized_kl_temp_filename(parameters):
    output_file_prefix = '../results/letter_A_kernelized_kl_temp_'
...
def generate_kernelized_postprocess_filename(parameters):
    output_file_prefix = '../results/letter_A_kernelized_postprocess_'
    return output_file_prefix + generate_data.combine_prefixes(
        settings.tsne_parameter_set | settings.letter_A_parameter_set, parameters)
...
def main(parameters = settings.parameters):
    ...
    kernelized_letters_percentiles_matrix = np.zeros((len(letter_A_samples), len(kernelized_method_list)))
    kernelized_letters_distance_matrix = np.zeros((len(letter_A_samples), len(kernelized_method_list)))
    ...
    with open(output_file, "wb") as f:
        pickle.dump((kernelized_method_list, kernelized_avg_letters_kl, kernelized_per_item_time, kernelized_letters_distance_percentiles),f)
