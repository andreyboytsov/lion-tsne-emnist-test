...
jobs_to_run = [i for i in range(latest_job + 1, latest_job + 1 + (80-num_jobs)) if i < 3300]
print(len(jobs_to_run), jobs_to_run)
for i in jobs_to_run:
   os.system('oarsub -n job'+str(i)+' -l nodes=1,walltime=3 "./single_chaos_job_script.sh '+str(i)+'"')
