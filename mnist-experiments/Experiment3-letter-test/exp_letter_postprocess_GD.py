...
def generate_gd_kl_temp_filename(parameters):
    output_file_prefix = '../results/letter_gd_kl_temp_'
...
def main(parameters = settings.parameters):
    ...
    input_time_file = exp_letter_test_GD.generate_time_results_filename(parameters)
    ...
    gd_time = [
        ...
        np.mean(letters_y_time_gd_variance_recalc_early_exagg_transformed_random),
    ]
    ...
    for i in range(len(letter_samples)):
        ...
        for j in range(len(gd_letters_results)):
            ...
            new_Y = np.concatenate((Y_mnist, gd_letters_results[j][i, :].reshape((1, -1))), axis=0)
            gd_letters_kl[j, i], _ = lion_tsne.kl_divergence_and_gradient(p_matrix=new_P, y=new_Y)
    ...
    gd_letters_percentiles_matrix = np.zeros((len(letter_samples), len(gd_method_list)))
    gd_letters_distance_matrix = np.zeros((len(letter_samples), len(gd_method_list)))
    for i in range(len(letter_samples)):
        for j in range(len(gd_method_list)):
            ...
            gd_letters_percentiles_matrix[i, j] = stats.percentileofscore(nearest_neighbors_y_dist, nn_dist)
    gd_letters_distance_percentiles = np.mean(gd_letters_percentiles_matrix, axis=0)
    gd_letters_distances = np.mean(gd_letters_distance_matrix, axis=0)
    for j in range(len(gd_method_list)):
        logging.info("%s: %f, %f", gd_method_list[j], gd_letters_distances[j], gd_letters_distance_percentiles[j])

    output_file = generate_gd_postprocess_filename(parameters)
    with open(output_file, "wb") as f:
        pickle.dump((gd_method_list, gd_time, gd_avg_letters_kl, gd_letters_distance_percentiles),f)
