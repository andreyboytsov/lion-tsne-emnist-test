...
outliers_y_multiquadric = all_RBF_IDW_LION_results["RBF-multiquadric"]['EmbeddedPoints']
outliers_y_gaussian = all_RBF_IDW_LION_results["RBF-gaussian"]['EmbeddedPoints']
outliers_y_linear = all_RBF_IDW_LION_results["RBF-linear"]['EmbeddedPoints']
outliers_y_cubic = all_RBF_IDW_LION_results["RBF-cubic"]['EmbeddedPoints']
outliers_y_quintic = all_RBF_IDW_LION_results["RBF-quintic"]['EmbeddedPoints']
outliers_y_inverse = all_RBF_IDW_LION_results["RBF-inverse"]['EmbeddedPoints']
outliers_y_thin_plate = all_RBF_IDW_LION_results["RBF-thin-plate"]['EmbeddedPoints']

outliers_y_idw1 = all_RBF_IDW_LION_results['IDW-1']['EmbeddedPoints']
outliers_y_idw10 = all_RBF_IDW_LION_results['IDW-10']['EmbeddedPoints']
outliers_y_idw20 = all_RBF_IDW_LION_results['IDW-20']['EmbeddedPoints']
outliers_y_idw40 = all_RBF_IDW_LION_results['IDW-40']['EmbeddedPoints']
outliers_y_idw_optimal = all_RBF_IDW_LION_results[idw_optimal_name]['EmbeddedPoints']

lion90_name = [i for i in all_RBF_IDW_LION_results.keys() if i.startswith('LION-90')][0]
outliers_y_lion90 = all_RBF_IDW_LION_results[lion90_name]['EmbeddedPoints']
lion95_name = [i for i in all_RBF_IDW_LION_results.keys() if i.startswith('LION-95')][0]
outliers_y_lion95 = all_RBF_IDW_LION_results[lion95_name]['EmbeddedPoints']
lion99_name = [i for i in all_RBF_IDW_LION_results.keys() if i.startswith('LION-99')][0]
outliers_y_lion99 = all_RBF_IDW_LION_results[lion99_name]['EmbeddedPoints']
lion100_name = [i for i in all_RBF_IDW_LION_results.keys() if i.startswith('LION-100')][0]
outliers_y_lion100 = all_RBF_IDW_LION_results[lion100_name]['EmbeddedPoints']
...
kernelized_tsne_method_list = [kernelized_detailed_tsne_method_list[i][:10]+kernelized_detailed_tsne_method_list[i][-8:]
                               for i in ind]
kernelized_tsne_outliers_results = [kernelized_detailed_tsne_method_results[i] for i in ind]
...
nn_results_file = exp_outlier_test_NN.generate_outlier_results_filename(parameters)
...
#cur_shown_outlier_indices = chosen_indices
#shown_outlier_indices = chosen_indices
#chosen_indices = [3]
...
h2 = ax[idw_Y][idw_X].scatter(outliers_y_idw10[:shown_outlier_indices, 0],
                outliers_y_idw10[:shown_outlier_indices, 1], c='blue', zorder=1, label=None, marker='.',s=point_size_interest)
h3 = ax[idw_Y][idw_X].scatter(outliers_y_idw20[:shown_outlier_indices, 0],
                outliers_y_idw20[:shown_outlier_indices, 1], c='green', zorder=1, label=None, marker='.',s=point_size_interest)
h4 = ax[idw_Y][idw_X].scatter(outliers_y_idw_optimal[:shown_outlier_indices, 0],
                outliers_y_idw_optimal[:shown_outlier_indices, 1], c='purple', zorder=1, label=None, marker='.',
                              s=point_size_interest)
h5 = ax[idw_Y][idw_X].scatter(outliers_y_idw40[:shown_outlier_indices, 0],
                outliers_y_idw40[:shown_outlier_indices, 1], c='cyan', zorder=1, label=None, marker='.',s=point_size_interest)
...
h2 = ax[gd_Y][gd_X].scatter(outliers_y_gd_transformed[cur_shown_indices,0],
                outliers_y_gd_transformed[cur_shown_indices,1], c='blue', marker='.',zorder=3,alpha=0.9,s=point_size_interest)
h3 = ax[gd_Y][gd_X].scatter(outliers_y_gd_transformed_random[cur_shown_indices,0],
                outliers_y_gd_transformed_random[cur_shown_indices,1], c='cyan', marker='.', zorder=3,alpha=0.9,
                            s=point_size_interest)
#h4 = ax.scatter(outliers_y_gd_variance_recalc_transformed[:cur_shown_indices,0],
#                outliers_y_gd_variance_recalc_transformed[:cur_shown_indices,1], c='green', marker='.', zorder=3,alpha=0.9)
#h5 = ax.scatter(outliers_y_gd_variance_recalc_transformed_random[:cur_shown_indices,0],
#                outliers_y_gd_variance_recalc_transformed_random[:cur_shown_indices,1],
#                c='brown', marker='.', zorder=3,alpha=0.9)
h6 = ax[gd_Y][gd_X].scatter(outliers_y_gd_early_exagg_transformed[cur_shown_indices,0],
                outliers_y_gd_early_exagg_transformed[cur_shown_indices,1], c='green', marker='.',zorder=3,alpha=0.9,
                           s=point_size_interest)
h7 = ax[gd_Y][gd_X].scatter(outliers_y_gd_early_exagg_transformed_random[cur_shown_indices,0],
...
h1 = ax[nn_Y][nn_X].scatter(nn_outliers_results[0][:cur_shown_outlier_indices, 0],
                nn_outliers_results[0][:cur_shown_outlier_indices, 1], c='red', zorder=1, label=None, marker='.',
                            s=point_size_interest)
h2 = ax[nn_Y][nn_X].scatter(nn_outliers_results[1][:cur_shown_outlier_indices, 0],
                nn_outliers_results[1][:cur_shown_outlier_indices, 1], c='blue', zorder=1, label=None, marker='.',
                           s = point_size_interest)
h3 = ax[nn_Y][nn_X].scatter(nn_outliers_results[2][:cur_shown_outlier_indices, 0],
...
ax[ktsne_Y][ktsne_X].scatter(Y_mnist[:, 0], Y_mnist[:, 1], c= 'gray', zorder=1, label=None, marker='.',
                            s = point_size_gray)
h1 = ax[ktsne_Y][ktsne_X].scatter(kernelized_tsne_outliers_results[0][:shown_outlier_indices, 0],
                kernelized_tsne_outliers_results[0][:shown_outlier_indices, 1], c='red', zorder=1, label=None, marker='.',
                                 s = point_size_interest)
h2 = ax[ktsne_Y][ktsne_X].scatter(kernelized_tsne_outliers_results[1][:shown_outlier_indices, 0],
                kernelized_tsne_outliers_results[1][:shown_outlier_indices, 1], c='blue', zorder=1, label=None, marker='.',
                                 s = point_size_interest)
h3 = ax[ktsne_Y][ktsne_X].scatter(kernelized_tsne_outliers_results[2][:shown_outlier_indices, 0],
                kernelized_tsne_outliers_results[2][:shown_outlier_indices, 1], c='green', zorder=1, label=None, marker='.',
                                 s = point_size_interest)
...
plt.savefig("../figures/outlier-test-all.png")
